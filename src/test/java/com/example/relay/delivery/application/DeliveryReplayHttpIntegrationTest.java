...
/**
 * Regression test for the Open-Session-In-View stale-read bug in
 * {@link DeliveryReplayService#replay}: with OSIV on (the project default -
 * spring.jpa.open-in-view is never overridden), one Hibernate Session is bound to the whole HTTP
 * request. The DeliveryStatus row loaded early in replay() lands in that session's identity map,
 * and because DeliveryStatus is {@code @Immutable}, a later findById for the "fresh" post-replay
 * row silently returned the same pre-replay instance without issuing any SQL.
 *
 * <p>
 * Neither {@code DeliveryReplayServiceTest} (pure Mockito - no real Session, no OSIV) nor
 * {@code DeliveryReplayLifecycleIntegrationTest} (direct method calls under plain
 * {@code @SpringBootTest} - each repository call gets its own transaction-scoped EntityManager, no
 * OSIV filter involved) can reproduce this: OSIV is a servlet-filter behaviour that only exists
 * when a request actually goes through the web layer. This test does that, via
 * {@code TestRestTemplate} against a real embedded server, which is the only way in this codebase
 * to install the real OSIV interceptor and observe the bug.
 */
    ...
    void replay_viaRealHttpRequest_returnsTheFreshPostReplayStatus_notTheStalePreReplaySnapshot() throws Exception {
        ...
        try {
            ...
            ResponseEntity<JsonNode> response = rest.exchange(
                    "/api/v1/environments/{environmentId}/apps/{appId}/deliveries/{deliveryId}/replay",
                    HttpMethod.POST, new HttpEntity<>(headers), JsonNode.class, env.getId(), app.getId(),
                    delivery.getId());
