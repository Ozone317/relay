...
public class AppControllerTest {
    ...
    void create_createsAndReturnsApp_ifItBelongsToTheEnvironmentAndUser() throws Exception {
        ...
        AppResponseDto response = new AppResponseDto(createdApp.getId(), createdApp.getName(),
                createdApp.getEnvironment().getId(), createdApp.getCreatedAt());
        ...
        // Act
        mockMvc.perform(post("/api/v1/environments/{environmentId}/apps", env.getId()).with(authentication(auth))
    ...
    @Test
    void getById_returnsApp_whenAppIsCreatedByUserAndEnvironmentIsCreatedByUserAndAppBelongsToTheEnvironment()
            throws Exception {
        // Arrange
        User user = new User("test@mail.com", "passwordHash");
        AuthenticatedUser principal = new AuthenticatedUser(user.getId(), user.getEmail());
        ...
        AppResponseDto response =
                new AppResponseDto(app.getId(), app.getName(), app.getEnvironment().getId(), app.getCreatedAt());
        ...
        // Act
        mockMvc.perform(get("/api/v1/environments/{environmentId}/apps/{appId}", env.getId(), app.getId())
    ...
    void getById_returns404_whenAppIsNotFound() throws Exception {
        ...
        // Stub
        when(appService.getById(app.getId(), env.getId(), user.getId()))
                .thenThrow(new AppNotFoundException(app.getId()));

        // Act + Assert
        mockMvc.perform(get("/api/v1/environments/{environmentId}/apps/{appId}", env.getId(), app.getId())
                .with(authentication(auth)).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound())
    ...
    void getAll_returnsAllApps_whenTheyBelongToTheEnvironmentAndCreatedByUser() throws Exception {
        ...
        List<AppResponseDto> response = List.of(
                new AppResponseDto(app1.getId(), app1.getName(), app1.getEnvironment().getId(), app1.getCreatedAt()),
                new AppResponseDto(app2.getId(), app2.getName(), app2.getEnvironment().getId(), app2.getCreatedAt()),
                new AppResponseDto(app3.getId(), app3.getName(), app3.getEnvironment().getId(), app3.getCreatedAt()));
        ...
        // Act
        mockMvc.perform(get("/api/v1/environments/{environmentId}/apps", env.getId()).with(authentication(auth))
    ...
    void delete_deletesTheApp_whenItExistsInTheEnvironmentAndIsCreatedByTheUser() throws Exception {
        ...
        // Act + Assert
        mockMvc.perform(delete("/api/v1/environments/{environmentId}/apps/{appId}", env.getId(), app.getId())
                .with(authentication(auth)).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isNoContent());
    ...
    void delete_throwsAppNotFoundException_whenAppDoesNotBelongToTheEnvironmentOrUser() throws Exception {
        ...
        // Stub
        doThrow(new AppNotFoundException(app.getId())).when(appService).delete(app.getId(), env.getId(), user.getId());
