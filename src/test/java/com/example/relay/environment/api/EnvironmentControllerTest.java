...
public class EnvironmentControllerTest {
    ...
    void create_createsAndReturnsEnvironment_whenUserIsAuthorizedAndRequestBodyIsCorrect() throws Exception {
        ...
        EnvironmentResponseDto response = new EnvironmentResponseDto(createdEnvironment.getId(),
                createdEnvironment.getName(), createdEnvironment.getDescription(), createdEnvironment.getUpdatedAt());
        when(environmentMapper.toResponseDto(createdEnvironment)).thenReturn(response);

        // Act + Assert
        mockMvc.perform(post("/api/v1/environments").with(authentication(auth)).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(requestBody))).andExpect(status().isCreated())
    ...
    void getById_returnsUserEnvironment_ifItExists() throws Exception {
        ...
        EnvironmentResponseDto response =
                new EnvironmentResponseDto(env.getId(), env.getName(), env.getDescription(), env.getUpdatedAt());
        ...
        // Act + Assert
        mockMvc.perform(get(apiPath).with(authentication(auth)).contentType(MediaType.APPLICATION_JSON))
    ...
    void getAll_returnsAllEnvironmentsOfUser_whenUserIsAuthorized() throws Exception {
        ...
        response.add(
                new EnvironmentResponseDto(env1.getId(), env1.getName(), env1.getDescription(), env1.getUpdatedAt()));
        response.add(
                new EnvironmentResponseDto(env2.getId(), env2.getName(), env2.getDescription(), env2.getUpdatedAt()));

        when(environmentService.getAll(userId)).thenReturn(envs);
        when(environmentMapper.toResponseDtoList(envs)).thenReturn(response);

        // Act + Assert
        mockMvc.perform(get("/api/v1/environments").with(authentication(auth)).contentType(MediaType.APPLICATION_JSON))
    ...
    void getById_returns404_whenEnvironmentNotFound() throws Exception {
        ...
        // Act + Assert
        mockMvc.perform(get("/api/v1/environments/{id}", missingId).with(authentication(auth)))
    ...
    void partialUpdate_updatesDescriptionAndReturnsEnvironment_whenValid() throws Exception {
        ...
        EnvironmentResponseDto response = new EnvironmentResponseDto(environment.getId(), environment.getName(),
                environment.getDescription(), environment.getUpdatedAt());
        when(environmentMapper.toResponseDto(environment)).thenReturn(response);

        // Act + Assert
        mockMvc.perform(patch("/api/v1/environments/{id}", environment.getId()).with(authentication(auth))
                .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(requestBody)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.description").value("Updated description"));
    ...
    void partialUpdate_returns400_whenDescriptionExceedsMaxLength() throws Exception {
        ...
        // Act + Assert
        mockMvc.perform(patch("/api/v1/environments/{id}", environmentId).with(authentication(auth))
                .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(requestBody)))
    ...
    void delete_deletesEnvironment_andReturnsNoContent() throws Exception {
        ...
        // Act + Assert
        mockMvc.perform(delete("/api/v1/environments/{id}", environmentId).with(authentication(auth)))
