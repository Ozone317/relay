-- H2-specific baseline for the default/test profile (H2 is rebuilt from scratch on every run).
-- Mirrors db/migration/postgresql/V1__baseline.sql; only the H2-native ENUM and JSON column
-- types differ from Postgres's varchar+check and jsonb.
...
CREATE TABLE attempts (
    ...
    status         enum ('CREATED','DEAD','FAILED_RETRYING','IN_FLIGHT','SCHEDULED','SUCCEEDED') NOT NULL,
